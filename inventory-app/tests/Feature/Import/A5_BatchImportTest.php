...
it('rolls back the entire file when strict mode input exceeds the chunk size and the final row fails', function (): void {
    $service = new ImportService();
    $category = Category::factory()->create(['name' => 'หมวดใหญ่']);
    $actor = User::factory()->create(['role' => 'admin']);
    ...
    expect(Product::query()->where('sku', 'LIKE', 'SKU-STRICT-L%')->exists())->toBeFalse();
