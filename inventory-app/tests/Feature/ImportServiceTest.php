...
test('ข้ามสินค้าเมื่อเลือกโหมด SKIP', function (): void {
    ...
    assertSameValue([], StockMovement::all());
});
...
test('ส่งออก error.csv เมื่อพบข้อมูลผิดพลาด', function (): void {
    ...
    $query = (string) parse_url($result['error_url'], PHP_URL_QUERY);
    parse_str($query, $params);
    $token = $params['token'] ?? null;
    assertNotNull($token);

    $path = $service->resolveErrorFilePath($token);
    assertTrue(is_file($path), 'error.csv file should exist');

    $csv = array_map('str_getcsv', file($path));
    assertSameValue('row_number', $csv[0][0]);
    assertSameValue('error_messages', $csv[0][1]);
    assertSameValue('raw_sku', $csv[0][2]);
    assertSameValue('2', $csv[1][0]);
    assertStringContains('sku:', $csv[1][1]);
    assertSameValue('INVALID SKU', $csv[1][2]);
});
